...
export PATH=$PATH:/usr/local/varnish/bin:/usr/local/varnish/sbin
...
# Simple Config File
cp etc/example.vcl /usr/local/varnish/etc/vcl.conf
...
start() {
	...
	-a 0.0.0.0:80 \
	-T 127.0.0.1:3500 \
	-f /usr/local/varnish/etc/default.vcl \
	-n /var/varnish_cache \
	-s malloc,512M \
	-P client_http11=on
        echo
...
cat > /usr/local/varnish/etc/default.vcl <<EOF
# This is a Varnish 4.x VCL file
vcl 4.0;

backend default {
    .host = "192.168.198.205";
    .port = "80";
    .probe = {
        .url = "/ping";
        .timeout   = 1s;
